...
def genere(version_courte=True, synthese=False):
    ...
    for i in range(len(carrieres)): # metiers
        ...
        for a in range(len(carrieres[i])): # age de début
            ...
            for g in range(len(carrieres[i][a])): # générations
                ...
                fic = "fig/%s_%d_%d.pdf"%(c.id_metier, c.annee_debut-c.age_debut, c.age_debut)
                    
                # Affichage synthétique
                for j in range(len(carrieres[i][a][g])): # modèles

                    c = carrieres[i][a][g][j]
                    f.write(dec("{\\bf \\noindent Retraites possibles et ratios Revenu/SMIC à 70, 75, 80, 85, 90 ans avec le modèle \\emph{"+modeles[j].nom+"}}  \n \n"))
                ...
                if not just_tex:
                    plt.savefig("./tex/"+fic)
                    plt.close('all')
                f.write("\n \\begin{center}\\includegraphics[width=0.95\\textwidth]{%s}\\end{center} \\label{%s} \n\n"%(fic,fic))
                f.write("\\newpage \n \n")                 
